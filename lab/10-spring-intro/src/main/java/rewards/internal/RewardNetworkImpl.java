...
public class RewardNetworkImpl implements RewardNetwork {
	...
	public RewardConfirmation rewardAccountFor(Dining dining) {
		...
		Account customerAccount = accountRepository.findByCreditCard(dining.getCreditCardNumber());
		Restaurant restaurant = restaurantRepository.findByMerchantNumber(dining.getMerchantNumber());
		MonetaryAmount monetaryAmount = restaurant.calculateBenefitFor(customerAccount, dining);
		AccountContribution contribution = customerAccount.makeContribution(monetaryAmount);
		accountRepository.updateBeneficiaries(customerAccount);
